...
function render(result){
 ...
 for(const repoNode of repoNodes){
  const prs=prsByRepo.get(repoNode.repository.id)||[],maxRank=Math.max(1,...prs.map(node=>node.rank)),lane=document.createElement('section');lane.className='repo-lane';lane.style.gridTemplateColumns=`repeat(${maxRank+1}, 290px)`;
  const cells=Array.from({length:maxRank+1},()=>[]);cells[0].push(repoNode);for(const node of prs)cells[Math.min(node.rank,maxRank)].push(node);
  lane.innerHTML=cells.map((nodes,rank)=>`<div class="lane-cell" data-rank="${rank}">${nodes.map(nodeHTML).join('')}</div>`).join('');lanes.append(lane)
 }
...
async function load(){if(loading)return;loading=true;document.body.classList.add('loading');progress.hidden=false;refresh.disabled=true;updated.textContent='Loading…';errorBox.hidden=true;try{const response=await fetch('/api/v1/graph?q='+encodeURIComponent(input.value));const body=await response.json();if(!response.ok)throw new Error(body.error||response.statusText);render(body);lastUpdated=Date.now();updated.textContent='Updated '+new Date(lastUpdated).toLocaleTimeString()}catch(error){errorBox.textContent=error.message;errorBox.hidden=false;updated.textContent=lastUpdated?'Update failed':'Load failed'}finally{loading=false;document.body.classList.remove('loading');progress.hidden=true;refresh.disabled=false;schedule()}}
